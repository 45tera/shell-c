...
void *pack_hotdog(void* arg){
	...
	while(1){
		...
		if (hotdog_pool.count == 0 && prod_done == 1) {
			pthread_mutex_unlock(&hotdog_mutex);
			return NULL;
		}
...
//follows instructor sourav's method - however, it does not take advantage of concurrency effectively.
